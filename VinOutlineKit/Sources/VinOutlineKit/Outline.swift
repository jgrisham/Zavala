...
public final class Outline: RowContainer, Identifiable, Equatable, Hashable, Codable {
	...
	func createNotes(rows: [Row], rowStrings: RowStrings?) async -> ([Row], Int?) {
		beginCloudKitBatchRequest()
	...
	@discardableResult
	public func deleteNotes(rows: [Row], rowStrings: RowStrings? = nil) async -> ([Row: NSAttributedString], Int?) {
		beginCloudKitBatchRequest()
	...
	@discardableResult
	public func deleteRows(_ rows: [Row], rowStrings: RowStrings? = nil) async -> Int? {
	...
	func createRow(_ row: Row, beforeRow: Row, rowStrings: RowStrings? = nil) async -> Int? {
		beginCloudKitBatchRequest()
		
		if let texts = rowStrings {
			await updateRowStrings(beforeRow, texts)
		}
		...
		if let parentRow = parent as? Row, await autoCompleteUncomplete(row: parentRow) {
			if let parentRowIndex = parentRow.shadowTableIndex {
	...
	func createRow(_ row: Row, afterRow: Row? = nil, rowStrings: RowStrings? = nil) async -> Int? {
		beginCloudKitBatchRequest()
		
		if let afterRow, let texts = rowStrings {
			await updateRowStrings(afterRow, texts)
		}
	...
	@discardableResult
	public func createRowsInsideAtStart(_ rows: [Row], afterRowContainer: RowContainer, rowStrings: RowStrings? = nil) async -> Int? {
		beginCloudKitBatchRequest()
	...
	@discardableResult
	public func createRowsOutside(_ rows: [Row], afterRow: Row, rowStrings: RowStrings? = nil) async -> Int? {
		beginCloudKitBatchRequest()
	...
	func splitRow(newRow: Row, row: Row, topic: NSAttributedString, cursorPosition: Int) async -> Int? {
		beginCloudKitBatchRequest()
		...
		let newCursorIndex = await createRows([newRow], afterRow: row)
	...
	func moveRowsRight(_ rows: [Row], rowStrings: RowStrings?) async -> [Row] {
		...
		for row in sortedRows {
			...
			await autoCompleteUncomplete(row: newParentRow)
	...
	func moveRowsLeft(_ rows: [Row], rowStrings: RowStrings?) async -> [Row] {
		...
		for row in rows.sortedWithDecendentsFiltered().reversed() {
			...
			if await autoCompleteUncomplete(row: oldParent) {
	...
	func moveRows(_ rowMoves: [RowMove], rowStrings: RowStrings? = nil) async {
		...
		for rowMove in sortedRowMoves {
			...
			if let parentRow = rowMove.toParent as? Row {
				await autoCompleteUncomplete(row: parentRow)
			}
		}
	...
	public func fixAltLinks() async {
		...
		if let keyedRows {
			...
			for row in keyedRows.values {
				let actionsTaken = await row.resolveAltLinks()
...
private extension Outline {
	...
	func autoCompleteUncomplete(row: Row) async -> Bool {
		...
		if row.isAutoUncompletable {
			await completeUncomplete(rows: [row], isComplete: false, rowStrings: nil)
			return true
		}
		...
		for row in rows {
			if let topic = row.topic {
				for linkToID in extractLinkToIDs(topic) {
					await deleteLinkRelationship(linkToID)
				}
			}
			if let note = row.note {
				for linkToID in extractLinkToIDs(note) {
					await deleteLinkRelationship(linkToID)
				}
			}
		}
	...
	func createLinkRelationships(for rows: [Row]) async {
		for row in rows {
			if let topic = row.topic {
				for linkToID in extractLinkToIDs(topic) {
					await createLinkRelationship(linkToID)
				}
			}
			if let note = row.note {
				for linkToID in extractLinkToIDs(note) {
	...
	func updateRowStrings(_ row: Row, _ rowStrings: RowStrings) async {
		...
		await row.apply(rowStrings)

		switch rowStrings {
		case .topicMarkdown, .topic:
			await processLinkDiff(oldText: oldTopic, newText: row.topic)
			replaceLinkTitleIfPossible(row: row, newText: row.topic, isInNotes: false)
		case .noteMarkdown, .note:
			await processLinkDiff(oldText: oldNote, newText: row.note)
	...
	func processLinkDiff(_ diff: CollectionDifference<EntityID>) async {
		...
		for change in diff {
			...
			case .insert(_, let entityID, _):
				await createLinkRelationship(entityID)
			case .remove(_, let entityID, _):
				await deleteLinkRelationship(entityID)
			}
		}
